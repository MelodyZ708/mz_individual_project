...
class SearchRunner:
    ...
    def evaluate(
        ...
        if failure_status is None:
            ...
            elif not self.keyframe_trajectory_source.is_file():
                evaluation.status = "FAIL_INVALID_TRAJECTORY"
                evaluation.reason = (
                    "COMO exited but did not produce historical results/data_tum.txt"
                )
            else:
                try:
    ...
    def print_result(self, candidate: Candidate, evaluation: Evaluation) -> None:
        if evaluation.status == "PASS":
            ...
            self.console.say(
                ...
                f"legacy_keyframe_Sim3_mean={evaluation.legacy_sim3_mean_m * 100:.4f}cm "
                f"legacy_scale={evaluation.legacy_sim3_scale:.4f} "
