...
class TumOdometryDataset(OdometryDataset):
    def __init__(self, seq_path, img_size, gt_tolerance=0.03):
        ...
        if os.path.exists(gt_path):
            self.pose_list, valid_indices = self.load_and_match_groundtruth(gt_path, self.ts_list)
            self.has_pose = True
            if len(valid_indices) < len(self.ts_list):
                skipped = len(self.ts_list) - len(valid_indices)
                print(f"[Info] Filtered {skipped} frames with no groundtruth match. "
                      f"Remaining: {len(valid_indices)}/{len(self.ts_list)}")
                self.ts_list   = [self.ts_list[i]   for i in valid_indices]
                self.rgb_list  = [self.rgb_list[i]  for i in valid_indices]
                if hasattr(self, "depth_list") and self.depth_list:
                    self.depth_list = [self.depth_list[i] for i in valid_indices]
    ...
    def load_and_match_groundtruth(self, gt_path, rgb_ts_list):
        ...
        pose_list = []
        valid_indices = []
        for i, ts in enumerate(rgb_ts_list):
            ...
            if best_idx is None or best_dt > self.gt_tolerance:
                print(f"[Warning] Skipping frame ts={ts:.6f}: no groundtruth within "
                      f"{self.gt_tolerance:.3f}s (best_dt={best_dt:.4f}s)")
                continue

            pose_list.append(gt_poses[best_idx])
            valid_indices.append(i)

        return pose_list, valid_indices
