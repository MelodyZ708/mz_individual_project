...
def track_and_init(
    ...
    if logz_m is None:
        dev = coords_j_m1_filt.device
        return torch.zeros(1,0,2,device=dev), torch.zeros(1,0,1,device=dev), torch.zeros(0,dtype=torch.bool,device=dev), torch.zeros(1,0,2,device=dev), torch.zeros(1,0,1,device=dev)
