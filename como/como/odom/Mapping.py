...
class Mapping:
    ...
    def init_basic_vars(self):
        ...
        print(f"[Mapping] color={self.cfg['color']}, channels c={c}")
    ...
    def add_keyframe(self, rgb, kf_pose_init, kf_aff_init, timestamp):
        ...
        # [TUM-VI] Skip keyframe if no correspondences found (e.g. fast motion)
        if coords_m_new.shape[1] == 0:
            return
    ...
    def get_img_and_grads(self, rgb):
        ...
        # update!
            ...
            img = self.feature_extractor(rgb)
    ...
    # 把所有 3D 锚点 P_m（世界坐标）投影到每个关键帧的图像坐标系，同时计算好所有雅可比矩阵。
    ...
    def iterate(self):
        ...
        #print(f"[Mapping] color={self.cfg['color']}, use_affine={self.cfg['color'] != 'cnn'}")
        # 重点！！！
        mean_sq_photo_err, self.kf_pairs, self.one_way_pairs = create_photo_system(
            ...
            dzm_dPwm,
            median_depths,
            vals_n, # gray pixel value -> DINO or CNN semantic vectors
            self.kf_img_and_grads,
        ...
        #print(f"[diag] delta nan={torch.isnan(delta).any().item()}, max={delta.abs().max().item():.4f}")
        #print(f"[diag] kf_poses nan={torch.isnan(self.kf_poses).any().item()}")
        #print(f"[diag] P_m nan={torch.isnan(self.P_m).any().item()}")
