#!/usr/bin/env bash
# Safe launcher for the resumable ResNet Layer2 full-sequence greedy search.
...
EVALUATOR="$SCRIPT_DIR/run_resnet_layer2_direct_fullseq_greedy.py"
OUTPUT_DIR="$PROJECT_ROOT/channel_selection_results/step_o_resnet_layer2_direct_fullseq_greedy"
...
printf '%s\n' '=============================================================================='
printf '[PLAN] Full fr1/desk_lightswitch only; maximum six selected post-ReLU Layer2 channels\n'
printf '[PLAN] Primary ranking: keyframe evo_ape ATE mean with alignment and scale correction\n'
printf '[PLAN] Same multi-start greedy / pair fallback / random / swap / repeats protocol as UNet Enc1\n'
printf '[PLAN] Per-run timeout: 300 seconds unless overridden; clean checkpoint every 36 h\n'
